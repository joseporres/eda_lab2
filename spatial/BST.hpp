...
class BSTree{
    ...
    void insert(BSNode<T>* &node,T data, bool (*lessThan)(const T&, const T&)){
        if(node){
    ...
    void insertBST(T data, bool (*lessThan)(const T&, const T&)){
        return insert(root,data, bool (*lessThan)(const T&, const T&));
    }
    std::vector<T> rangeSearch(T lb, T ub, bool (*lessThan)(const T&, const T&)){
        std::vector<T>points;
